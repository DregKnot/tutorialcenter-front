...
export default function CourseAdvisorGuardianManagement() {
  ...
  const [stats, setStats] = useState([
    { label: "Total Guardians", value: 0, icon: UserGroupIcon, bg: "bg-[#09314F]/10", color: "text-[#09314F]" },
    { label: "Active", value: 0, icon: ShieldCheckIcon, bg: "bg-[#BB9E7F]/10", color: "text-[#BB9E7F]" },
  ]);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_BASE_URL}/api/advisor/guardians/all`, {
      ...
      setStats([
        ...
        { label: "Active", value: data.length, icon: ShieldCheckIcon, bg: "bg-[#BB9E7F]/10", color: "text-[#BB9E7F]" },
      ]);
    } catch (error) {
      console.error("Failed to fetch guardians for advisor:", error);
    } finally {
      setLoading(false);
    }
  }, [API_BASE_URL, token]);
  ...
  return (
    <StaffDashboardLayout pagetitle="GUARDIAN MANAGEMENT">
      ...
      {/* Student View Modal if advisor clicks View Profile on any linked ward */}
      {selectedStudentId && (
