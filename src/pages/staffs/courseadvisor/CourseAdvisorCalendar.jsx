...
export default function CourseAdvisorCalendar() {
  ...
  const handleJoinClass = useCallback((s) => {
    ...
    if (isZoom && s.id) {
      setJoiningSessionId((prev) => (prev === s.id ? null : s.id));
    } else if (link) {
  ...
  const fetchSchedule = useCallback(async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_BASE_URL}/api/advisor/classes/schedule`, {
        headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
      });
      const flatList = getFlatSessions(res.data || {});
      setSessions(flatList);
    } catch (error) {
      console.error("Failed to fetch advisor calendar schedule:", error);
      setSessions([]);
    } finally {
      setLoading(false);
    }
  ...
  // Save updated video recording link
  ...
  return (
    <StaffDashboardLayout pagetitle="Calendar">
                            ...
                            joiningSessionId === s.id ? (
                              <div className="flex flex-col gap-2 w-full animate-in fade-in duration-200">
                                ...
                                <button
                                  ...
                                  className="text-[10px] text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 font-semibold py-0.5 text-center transition-colors"
                                >
                                  Cancel
                                </button>
                              </div>
      ...
      {/* ====== DETAIL MODAL (With Update Recording manager) ====== */}
      {selectedSession && (
          ...
          <div className="relative bg-white dark:bg-[#092238] rounded-[32px] p-8 md:p-10 w-full max-w-md shadow-2xl animate-in fade-in zoom-in-95 duration-200 border border-slate-100 dark:border-white/5">
            ...
            <div className="space-y-6">
              ...
              <div className="border-t border-slate-100 dark:border-white/5 pt-6 mt-6">
                <label className="block text-xs font-black text-[#0F2843] dark:text-blue-300 uppercase tracking-widest mb-3">
                  Upload Recording Link
                </label>
                <div className="relative group mb-4">
                  <Icon icon="mdi:video" className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 group-focus-within:text-[#0F2843] transition-colors" />
                  <input
                    type="text"
                    placeholder="https://youtube.com/watch?v=..."
                    value={videoLink}
                    onChange={(e) => setVideoLink(e.target.value)}
                    className="w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-black/20 border border-gray-250 dark:border-white/10 rounded-2xl text-[14px] font-bold text-[#1F2937] dark:text-white focus:outline-none focus:ring-2 focus:ring-[#0F2843]/10 focus:border-[#0F2843] transition-all"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full bg-[#0F2843] text-white py-3.5 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-[#E83831] disabled:bg-slate-300 transition-all flex items-center justify-center gap-2 active:scale-98"
                >
                  {saveLoading && <Icon icon="line-md:loading-twotone-loop" className="w-4 h-4" />}
                  Save Recording Link
                </button>
              </div>
            </div>
